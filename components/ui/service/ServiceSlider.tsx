    ...
    <div className="mt-12">
      <Swiper
        ...
        breakpoints={{
          ...
          // Increased spacing below to prevent the scaled card (1.3x) from hitting neighbors
          1024: { slidesPerView: 3, spaceBetween: 100 },
          1440: { slidesPerView: 5, spaceBetween: 110 },
        }}
        // 👆
      >
        {services.map((service) => (
          ...
          >
            {({ isActive, isPrev, isNext }) => {
              ...
              return (
                <motion.div
                  style={{ scale }}
                  // Added z-index logic to ensure the active card floats ABOVE neighbors if they do overlap slightly
                  className={`my-10 flex-shrink-0 w-72 h-52 rounded-[20px] p-6 flex flex-col items-center bg-[#F1F2FA] text-center transition-all duration-500 relative
          ${
            isActive
              ? " shadow-md z-10 " // Added z-10
              : isPrev || isNext
              ? " text-black z-0 "
              : " text-black/70 z-0 "
          }`}
